...
- UI ASCII 线框：`docs/ui-ascii-wireframes.md`。
...
- Layout Preset 的完整创建、命名、保存、复用、删除流程需等待 Milestone 2 完成后详细规划。
...
- Milestone 2 中，编辑网格后的布局变化应尽可能对已打开的当前 MultiviewWindow 动态生效，无需重启 OBS。
- Milestone 3 中，添加 / 更换 / 清空 cell 后，当前 MultiviewWindow 应动态更新，无需重启 OBS。
- Milestone 4 中，视觉参数修改应动态生效；具体规则需等待 Milestone 3 完成后详细规划。
- Milestone 5 中，断流策略修改应动态生效；具体规则需等待 Milestone 3 完成后详细规划。
...
- 需要在 Milestone 3 中结合 OBS 31.0.0 到 32.1 的实际 API 与仓库结构确认具体实现方式。
...
- 管理配置加载与保存。
- 管理实例打开、聚焦、关闭、克隆、删除。
- 管理 Layout Preset 数据结构；完整 UI 需等待 Milestone 2 完成后详细规划。
...
- 第一阶段仍以 Multiview 实例保存布局为主。
- Layout Preset 的完整管理流程需等待 Milestone 2 完成后详细规划。
...
17. Layout Preset 的完整创建、命名、保存、复用、删除流程需等待 Milestone 2 完成后详细规划；第一阶段仍以 Multiview 实例保存布局为主。
...
- 结合 OBS 31.0.0 到 32.1 的实际 API 与仓库结构确认 Qt 窗口句柄和独立渲染管线的具体实现方式。
...
- 性能限制。
- 与保存网格信号的关系。
- 视觉参数修改后如何对当前 MultiviewWindow 动态生效。
