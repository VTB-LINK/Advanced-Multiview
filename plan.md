...
> 对尚未详细讨论的未来阶段，本文只保留方向性占位，并明确标注“需等待 Phase 2.5 完成后详细规划”。
...
| Phase 2.5（M4 收尾 / Phase 3 准备） | 进行中 | 文档重基线、验收清单、术语统一、VU meter polish 设计；不是新的全局 Milestone，仅收尾窗口 |
...
- Layout Preset 的完整创建、命名、保存、复用、删除流程不属于 Phase 2 主体，需等待 Phase 2.5 之后再单独规划。
...
- Milestone 5 中，断流策略修改应动态生效；具体规则需等待 Phase 2.5 完成后详细规划。
...
- 管理配置加载与保存。
- 管理实例打开、聚焦、关闭、克隆、删除。
- 管理 Layout Preset 数据结构；完整 UI 不属于 Phase 2 主体，需在 Phase 2.5 之后单独规划。
...
- 第一阶段仍以 Multiview 实例保存布局为主。
- Layout Preset 的完整管理流程不属于 Phase 2 主体，需在 Phase 2.5 之后单独规划。
...
未来 provider 需等待 Phase 2.5 完成后、随 Phase 3（M5~M6）详细规划。
...
17. Layout Preset 的完整创建、命名、保存、复用、删除流程不属于 Phase 2 主体，需在 Phase 2.5 之后单独规划；第一阶段仍以 Multiview 实例保存布局为主。
...
状态：**主体已完成（Phase 2）**；当前在 **Phase 2.5（M4 收尾 / Phase 3 准备）** 进行验收、文档同步与少量必要 polish。详见：
...
- VU meter polish 设计：peak hold、dB 标尺/刻度、scene/source cell 的 trackMode 语义决策。
...
状态：未开始，需等待 Phase 2.5 完成后详细规划；部分内容也依赖 Milestone 5。
