...
- 前景叠加图片。
- PGM/PRVW 高亮边框。
- PGM/PRVW 高亮支持**嵌套场景检测**，嵌套命中的边框样式可选（虚线、实线或无边框）。
- **VU Meter。**
- VU 峰值保持。
- VU dB 刻度和标签。
- VU 根据源声道数显示**多声道电平条**。
...
多视图是监看工具，所以本插件遵循一条底线：**绝不让 OBS 崩溃、卡顿或阻塞**——OBS 线程上的任何故障都会拖垮整场直播。

- **渲染路径绝不调用 OBS Frontend API。** Program / Preview / 推流状态都从 OBS 主线程更新的缓存快照读取，绘制单元格不会与场景切换发生竞争。
- **每个实例只拉一次流。** 一个实例的所有窗口共用同一组源和 VU meter；为同一个多视图再开一个窗口，不会重复拉取信号。
- **没有窗口时输出照常运行。** 开启外部输出后，即使一个实例的窗口全部关闭，它仍作为 headless host 存活并持续发送。
- **拆除过程是 use-after-free 安全的。** 关闭窗口或切换场景集合时，会先把实例从渲染驱动里摘除，再销毁任何资源。

性能方面的改动让多视图在繁忙直播时不挤压主输出：

- **多视图窗口渲染帧率（Full / Half，默认 Half）。** Half 以一半的基准帧率合成网格、其余帧复用上一帧，大致把每个窗口的渲染开销减半；仅在高于 30 fps 时提供。
- **无接收端时 NDI 输出空转。** 没有任何端拉取输出时，跳过 GPU 回读与编码，同时发送器保持可被发现，直到有接收端连入。
- **可选的 NDI 回读双缓冲**（默认开启）：以一帧的代价保护主输出不被慢速 GPU 的回读卡顿影响；关闭则获得最低延迟与更紧的音画同步。

## 环境要求

- OBS Studio 31.1.1 或更新版本。
- Windows 是当前主要测试平台。
- 外部信号提供器**单元格**需要可选宿主插件：
  - NDI 单元格需要 DistroAV
...
内置的 **NDI 外部输出**（视频 + 音频）在 **Windows、macOS 与 Linux** 上开箱即可构建：[NDI SDK](https://ndi.video/for-developers/ndi-sdk/) 的头文件已 vendored 于 [`deps/ndi/`](deps/ndi/README.md)（仅头文件——不含导入库、不打包库），因此 CI 与贡献者均无需安装 SDK。若想改用本机已装的 SDK，设置环境变量 `NDI_SDK_DIR`（或安装到默认路径）即可，其优先级高于 vendored 头。
