...
- 术语权威基准：[TERMINOLOGY.md](TERMINOLOGY.md)（Phase / Milestone 命名以本文件为准）。
- Phase 2 视觉参数设计：[phase-2-visual-settings-design.md](phase-2-visual-settings-design.md)。
- Phase 2 代码硬化记录：[phase-2-hardening-notes.md](phase-2-hardening-notes.md)。
- Phase 3 Signal Lost 与外部信号设计：[phase-3-signal-lost-and-external-sources-design.md](phase-3-signal-lost-and-external-sources-design.md)。
- Phase 1 验收清单：[phase-1-acceptance-checklist.md](phase-1-acceptance-checklist.md)。
- Phase 1 代码硬化记录：[phase-1-hardening-notes.md](phase-1-hardening-notes.md)。
- 已知限制：[known-limitations.md](known-limitations.md)。
- UI ASCII 线框：[ui-ascii-wireframes.md](ui-ascii-wireframes.md)。

---

## 当前阶段状态

以 [TERMINOLOGY.md](TERMINOLOGY.md) 中固化的术语为准：

| 编号 / Phase | 状态 | 备注 |
| --- | --- | --- |
| M0 ~ M3 / Phase 1 | 功能闭环已完成 | 详见 [phase-1-acceptance-checklist.md](phase-1-acceptance-checklist.md)；OBS 32.0 / Linux / macOS 运行时尚未完全勾选，tag release workflow 已开始验证 |
| M4 / Phase 2 | 主体功能已完成 | Label、Background、SafeArea、Overlay、VU Meter、PGM/PRVW Highlight、三层 Visual Settings、动态生效；详见 [phase-2-visual-settings-design.md](phase-2-visual-settings-design.md) 与 [phase-2-hardening-notes.md](phase-2-hardening-notes.md) |
| Phase 2.5（M4 收尾 / Phase 3 准备） | 已完成 | 文档重基线、验收清单、术语统一、VU meter polish 设计/实现均已完成；不是新的全局 Milestone，仅收尾窗口 |
| M5 / Phase 3 上半段 | 功能完成 | Signal Lost 完整运行时（Black / PlaceholderImage / ClearCell）+ Fallback (PGM/PRVW/Scene/Source/Image) + Reconnect Now + 动态生效；详见 [phase-3-acceptance-checklist.md](phase-3-acceptance-checklist.md) |
| M6 / Phase 3 下半段 | 功能完成 | 外部流接入：DistroAV NDI · obs-spout2 Spout · FFmpeg · VLC；WebRTC 预留占位。外部 cell 全链路已进入 retry-with-fallback / sticky overlay / paused / play·pause·next·previous 右键流，并完成 Detailed logs / restart-before-recreate / Reconnect Now 对齐等收尾硬化。详见 [phase-3-acceptance-checklist.md](phase-3-acceptance-checklist.md) M6 章节 |
| M7 / M8 / Phase 4 | 进行中 | Release workflow、Windows portable/user-layout artifact、Ubuntu artifact、source tarball 已接通；仍需补齐安装器策略、跨平台运行时验证、性能稳定性回归 |
| Phase 3 后增强（issue #10 / #11） | 主体完成，归入 M7/M8 范围 | 多投影窗口（issue #10）、NDI/Spout 外部输出（issue #11）、广播级隔离与性能硬化已落地；**非新增全局 Milestone**，详见 §14 |
...
- 支持自定义网格数量与 span 合并。
- 支持多个独立 Multiview 实例。
- 支持窗口化与全屏显示，并支持**为同一个实例同时打开多个投影窗口**，共用同一组源（issue #10）。
- 第一阶段支持 OBS 内部信号：PGM、PRVW、Scene、Source。
- 已支持外部信号：FFmpeg media（RTMP / HLS / FLV / SRT / 本地文件等）、DistroAV NDI、obs-spout2 Spout、VLC playlist。WebRTC 当前仅保留占位与持久化兼容。
- 已支持**将合成后的多视图作为外部输出发送**：NDI（视频 + 音频）与 Spout（Windows，仅视频），独立于 OBS 场景系统（issue #11）。
...
> **已更新（issue #10）**：第一阶段"每个实例只允许一个窗口"的限制已被取消。现在一个实例可同时打开**多个投影窗口**——它们是同一个 `AmvInstanceCore`（共享源 / volmeter / 图像资源 / 外部输出）之上的轻量 view，各自有独立的 OBSDisplay 与布局尺寸，但**只拉一次流**。窗口标题以 "… | 窗口 n" 区分。编辑仍集中在管理 / 设置页面，所以下面关于"多可编辑窗口同步"的顾虑依然成立——N 个窗口是只读投影，而不是 N 份可独立编辑的副本。详见 §14.1。

第一阶段决策（历史，部分已被 issue #10 取代）：

- ~~一个 Multiview 实例同一时间只允许一个可编辑窗口。~~ 已取消：现支持多投影窗口。
- 如果用户重复打开同一个实例，可在管理页面再开一个投影窗口；如果需要真正不同的布局 / 设置，则克隆整个 Multiview 实例。
- 克隆后生成新的实例 UUID，并可独立编辑、保存和打开。

仍不做：

- 同一个 Multiview 实例打开多个各自独立、互不同步的**可编辑**窗口（编辑集中在管理页面）。
- 多窗口冲突合并。

原因：

- 多窗口同时编辑同一份实例会引入复杂的 dirty 状态、覆盖顺序、保存冲突和同步问题。
- issue #10 的多窗口是共享同一 core 的只读投影 view，规避了上述同步问题。
...
- Milestone 4 中，视觉参数修改应动态生效。**（已完成）** 由 `MultiviewWindow::refresh_visual_settings()` + `notify_multiview_visual_settings_changed()` 触发；详见 [phase-2-visual-settings-design.md](phase-2-visual-settings-design.md) 第 15 节。
...
- 当前不在 ROADMAP.md 中无依据指定具体渲染上下文实现方式。
...
- 代码硬化记录：[phase-2-hardening-notes.md](phase-2-hardening-notes.md)
...
- 文档重基线：本文件、[../README.md](../README.md)、[known-limitations.md](known-limitations.md)、新增 `docs/phase-2-acceptance-checklist.md`。
- 术语统一：以 [TERMINOLOGY.md](TERMINOLOGY.md) 为准，避免 Phase / Milestone 互换。
- VU meter polish 设计与实现：Peak Hold、dB 标尺/刻度、Show Labels、Scale Side、scene/source cell 的 trackMode 语义决策与 Cell scope 禁用。
- 归档 [phase-2-hardening-notes.md](phase-2-hardening-notes.md) 观察项到验收清单或后续 issue。
...
状态：**功能完成**。详细设计基准见 [phase-3-signal-lost-and-external-sources-design.md](phase-3-signal-lost-and-external-sources-design.md)，验收记录见 [phase-3-acceptance-checklist.md](phase-3-acceptance-checklist.md)。
...
- 文案：`MISSING SOURCE` / `MISSING SCENE` / `FALLBACK` 已上线；`SIGNAL LOST` / `CONNECTING...` 已在 M6 外部源状态路径补齐。
...
- 外部源 SignalLost / RetryWithFallback / Connecting 状态实际渲染。
...
状态：**功能完成**。详细设计基准见 [phase-3-signal-lost-and-external-sources-design.md](phase-3-signal-lost-and-external-sources-design.md) §8，执行级验收清单见 [phase-3-acceptance-checklist.md](phase-3-acceptance-checklist.md) M6 章节。
...
- ✅ M6.6：Phase 3 综合硬化：external cell lifecycle（sticky fallback latch · retry-on-all-behaviors · 避免 Active 闪烁 · image fallback during FallbackActive · row-wise pillarbox snap · instance-prefixed log · Paused state）；M5 外部 cell fallback 接通；外部 VU 与 trackMode 正交；Detailed logs 统一入口；Lost 路径 restart-before-recreate；Reconnect Now 与 supervisor 恢复路径对齐。
...
状态：**已完成**。以下三项均已落地并通过 Windows RelWithDebInfo + OBS 31.1.1 / 32.1.2 portable 人工验证。
...
该组任务已完成；后续 M6.2~M6.6 已在此基础上继续落地。

---

## 10. Milestone 7：打包、安装版与便携版 artifacts

状态：**部分推进**。Release workflow 已能产出 Windows portable / user-layout zip、Ubuntu artifact 与 source tarball（详见 [known-limitations.md](known-limitations.md) §分发）；Windows installer 形式、跨平台运行时验证与发布矩阵仍待补齐。
...
状态：**部分推进**。性能与稳定性硬化已通过 issue #10 / #11 落地一部分（广播级隔离、四阶段拆除、多视图窗口渲染帧率 Full/Half、无接收端 NDI 空转、高亮遍历节流、可选 NDI 回读双缓冲；详见 §14.3）；形式化的回归矩阵（6x6 / 10x10 性能目标、source 删除 / 改名 / 场景集合切换回归清单）与跨平台运行时验证仍未开始。
...
状态：**Phase 1（M0~M3）最小闭环已完成**。本节作为历史基线保留，验收证据见 [phase-1-acceptance-checklist.md](phase-1-acceptance-checklist.md)。
...
## 14. Phase 3 之后的增强（issue 跟踪，非新增 Milestone）

> 以下增强在 Phase 3（M5~M6）主体完成后通过 GitHub issue 跟踪推进，**不新增全局 Milestone**（遵循 [TERMINOLOGY.md](TERMINOLOGY.md)：Phase 上限 Phase 4，不扩展 Milestone 编号）。它们主要落在 M7/M8 的工程与性能稳定性范围内，或属于 PRD 既有方向的能力扩展。

### 14.1 多投影窗口（issue #10）

- 将原 MultiviewWindow 拆为 core / view：每个实例一个 `AmvInstanceCore`（独占共享源、volmeter、图像资源、外部输出、渲染），其上挂载 N 个轻量 `MultiviewWindow` view（各自 OBSDisplay + 布局尺寸）。
- 同一实例多窗口**只拉一次流、只建一组 volmeter**；窗口标题以 "… | 窗口 n" 编号。
- 所有窗口关闭后，如外部输出仍开启，core 作为 **headless host** 存活并继续发送。
- 取代了 §1.2 的"每实例单窗口"限制；编辑仍集中在管理 / 设置页面（N 个窗口是只读投影）。

### 14.2 外部输出：Spout / NDI（issue #11，Approach B）

- 将合成后的多视图作为**发送端**输出，独立于 OBS 的 source / scene 系统：Spout（Windows，仅视频）与 NDI（视频 + 音频）。
- 后端无关的输出层（`MultiviewOutputManager` + per-backend）：每个后端各自选择输出分辨率与帧率 divisor，相同尺寸的后端共享一次离屏合成。
- NDI 音频源可选：跟随推流轨道 / 手动轨道 / 无。
- 不链接、不打包 NDI SDK：头文件 vendored 于 `deps/ndi/`，运行时**动态加载** NDI 运行时。

### 14.3 广播级隔离与性能硬化（贯穿 issue #10 / #11，归入 M8 范围）

详尽审计记录见 [obs-isolation-review.md](obs-isolation-review.md)。要点：

- **渲染线程零 `obs_frontend_*` 调用**：PGM / PRVW / 推流状态走主线程更新的缓存快照，消除与场景切换的竞争（F2）。
- **四阶段拆除 + UAF 安全**：core 在销毁前先移出渲染驱动列表；锁顺序 `g_registry → 图形锁 → source_mutex_`，渲染线程从不取 registry 锁（F5）。
- **性能**：多视图窗口渲染帧率 Full / Half（默认 Half，仅 >30fps 提供）；无接收端时 NDI 输出空转、跳过合成 / 回读 / 编码（发送端仍可被发现）；PGM / PRVW 高亮场景树遍历节流；可选 NDI 回读双缓冲（默认开）。
- 形式化的 M8 回归矩阵（6x6 / 10x10 性能目标、source 删除 / 改名 / 场景集合切换回归清单、跨平台运行时）仍待完成（见 §11）。
